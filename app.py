...
if input_file:
    ...
    if src_df is not None:
        if marketplace_type == "General":
            ...
            with col1:
                selected_variant_col = st.selectbox("Seller SKU → variantId (leave '(none)' to skip)", options=cols, index=0)
            with col2:
                selected_product_col = st.selectbox("Style Code → productId (leave '(none)' to skip)", options=cols, index=0)
        else:
